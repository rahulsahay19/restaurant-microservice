...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment,
                                                   ...
                                                   List<String> failureMessages, DomainEventPublisher<PaymentCompletedEvent> paymentCompletedEventDomainEventPublisher,
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        if(payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())){
            log.error("Customer with id: {} doesn't have enough credit to make the payment", payment.getCustomerId().getValue());
            //Accumulating all the failure messages. And in the end all these failure messages will be returned to the client
            failureMessages.add("Customer with id=" + payment.getCustomerId().getValue() + "doesn't have enough credit to make the payment");
        }
    }
